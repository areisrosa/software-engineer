...
public class BinarySearchST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Returns the number of keys in this symbol table strictly less than {@code
     * key}
     *
     * @param  key the key
    ...
    ***************************************************************************/
    ...
    public void randomSearchHit() {
        VisualAccumulator acc = new VisualAccumulator(n, 2*Math.log(n)/Math.log(2));
        for (Key key: this.keys()) {
            get(key);
            StdOut.println(getHits());
            acc.addDataValue(getHits());
        }
        StdOut.println(acc); // invokes the method toString acc
    }
